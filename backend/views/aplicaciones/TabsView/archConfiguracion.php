...
<?= $form->field($model, 'AppObse4')->textarea(['maxlength' => true, 'disabled' => true, 'rows' => '3']) ?>
<div class="row">
        ...
        <?php foreach ($modelsApparchivos as $i => $modelApparchivos): ?>
          <div class="item4 panel panel-default"><!-- widgetBody Se agrega el número 4 para diferenciarlo de otros similares DENTRO DEL MISMO FORMULARIO (Debe de ser constante en todo el widget)-->
            ...
            <div class="panel-body">
              ...
              <div class="row">
                <div class="col-sm-6">
                  <?= $form->field($modelApparchivos, "[{$i}]AArcDirec")->textInput(['maxlength' => true, 'disabled' => true]) ?>
                </div>
                <div class="col-sm-6">
                  <?= $form->field($modelApparchivos, "[{$i}]AArcNombArch")->textInput(['maxlength' => true, 'disabled' => true]) ?>
                </div>
              </div><!-- .row -->
              <div class="row">
                <div class="col-sm-6">
                  <?= $form->field($modelApparchivos, "[{$i}]AArcVari")->textInput(['maxlength' => true, 'disabled' => true]) ?>
                </div>
                <div class="col-sm-6">
                  <?= $form->field($modelApparchivos, "[{$i}]AArcNombVari")->textInput(['maxlength' => true, 'disabled' => true]) ?>
                </div>
              </div><!-- .row -->
              <div class="row">
                <div class="col-sm-12">
                  <?= $form->field($modelApparchivos, "[{$i}]AArcDescPara")->textarea(['maxlength' => true, 'disabled' => true, 'rows' => '2']) ?>
                </div>
              </div><!-- .row -->
            </div>
          </div>
