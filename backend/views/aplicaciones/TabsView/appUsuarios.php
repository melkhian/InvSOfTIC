...
<?= $form->field($model, 'AppObse6')->textInput(['maxlength' => true, 'disabled' => true]) ?>
      ...
      <?php foreach ($modelsAppusuarios as $i => $modelAppusuarios): ?>
        <div class="item3 panel panel-default"><!-- widgetBody Se agrega el número 1 para diferenciarlo de otros similares DENTRO DEL MISMO FORMULARIO (Debe de ser constante en todo el widget)-->
          ...
          <div class="panel-body">
            ...
            ?>
            <?= $form->field($modelAppusuarios, "[{$i}]AUsuUsua")->textInput(['maxlength' => true, 'disabled' => true]) ?>
            <div class="row">
              <div class="col-sm-12">
                <?= $form->field($modelAppusuarios, "[{$i}]AUsuDesc")->textarea(['maxlength' => true, 'disabled' => true, 'rows' => '2']) ?>
              </div>
            </div><!-- .row -->
          </div>
        </div>
