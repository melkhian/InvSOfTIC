...
<br><br>
<?= $form->field($model, 'AppDireRaiz')->textInput(['maxlength' => true, 'disabled' => true]) ?>
<?= $form->field($model, 'AppObse5')->textarea(['maxlength' => true, 'rows' => '3', 'disabled' => true]) ?>
      ...
      <?php foreach ($modelsAppdirectorios as $i => $modelAppdirectorios): ?>
        <div class="item2 panel panel-default"><!-- widgetBody Se agrega el número 1 para diferenciarlo de otros similares DENTRO DEL MISMO FORMULARIO (Debe de ser constante en todo el widget)-->
          ...
          <div class="panel-body">
            ...
            ?>
            <?= $form->field($modelAppdirectorios, "[{$i}]ADirDirec")->textInput(['maxlength' => true, 'disabled' => true]) ?>
            <div class="row">
              <div class="col-sm-12">
                <?= $form->field($modelAppdirectorios, "[{$i}]ADirDesc")->textarea(['maxlength' => true, 'rows' => '2', 'disabled' => true]) ?>
              </div>
            </div><!-- .row -->
          </div>
        </div>
