<?php

namespace backend\models;

use Yii;
use yii\base\Model;
use yii\data\ActiveDataProvider;
use backend\models\Apphardware;

/**
 * ApphardwareSearch represents the model behind the search form of `backend\models\Apphardware`.
 */
class ApphardwareSearch extends Apphardware
{
    /**
     * @inheritdoc
     */
    public function rules()
    {
        return [
            [['AHarId', 'AppId_fk'], 'integer'],
            [['AHarTipoHard', 'AHarProc', 'AHarMemo', 'AHarEspaDisc'], 'safe'],
        ];
    }

    /**
     * @inheritdoc
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params)
    {
        $query = Apphardware::find();

        // add conditions that should always apply here

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }

        // grid filtering conditions
        $query->andFilterWhere([
            'AHarId' => $this->AHarId,
            'AppId_fk' => $this->AppId_fk,
        ]);

        $query->andFilterWhere(['like', 'AHarTipoHard', $this->AHarTipoHard])
            ->andFilterWhere(['like', 'AHarProc', $this->AHarProc])
            ->andFilterWhere(['like', 'AHarMemo', $this->AHarMemo])
            ->andFilterWhere(['like', 'AHarEspaDisc', $this->AHarEspaDisc]);

        return $dataProvider;
    }
}
